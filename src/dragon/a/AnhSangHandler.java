...
public class AnhSangHandler {

    /*
    - Bộ 3: Tổng chỉ số buff từ trang bị Ánh Sáng được tăng nhẹ
    - Bộ 6: Kích hoạt Thánh Quang. Tổng chỉ số buff từ trang bị Bóng Tối tăng mạnh. Giảm sát thương từ Boss và Quái Ánh Sáng
    - Thánh Quang: Trong khoảng thời gian ban ngày (từ 5h sáng đến 16h chiều), trang bị Ánh Sáng sẽ được tăng cường sát thương gây ra cho boss, 
    quái Bóng Tối, tăng HP và KI, giảm nhẹ SĐ.
    ...
    - Trong khoảng thời gian ban ngày (5h sáng đến 15h chiều)
     ...
     */
    private static List<Integer> option_item = Arrays.asList(251, 252, 253);
    private static int id_option_level = 254;
    private static final int id_option_set_bo_3 = 232;
    private static final int id_option_set_bo_6 = 233;
    private static final int id_option_noi_tai = 234;
    private static final int id_map_quai_bong_dem = 0;
    private static final int id_boss_bong_dem = 0;

    public static int PARAM_HP;
    public static int PARAM_KI;
    public static int PARAM_SD;

    public static int countAnhSangItems(Char charz) {
        if (charz == null) {
            return 0;
        }
    ...
    public static boolean isSet3AnhSang(Char charz) {
        if (charz == null) {
            return false;
        }
        return countAnhSangItems(charz) >= 3;
    }

    public static boolean isSet6AnhSang(Char charz) {
        if (charz == null) {
            return false;
        }
        return countAnhSangItems(charz) >= 6;
    }
    ...
    public static void updateOptionNoiTaiByTime(Char charz, int hoursNow) {
        for (Item trangBi : charz.arrItemBody) {
            ...
            boolean hasAnhSang = trangBi.options.stream()
    ...
    private static void updateSetOption(boolean isSetActive, int optionId, Char charz) {
        ...
        for (Item trangBi : charz.arrItemBody) {
            ...
            if (isSetActive && hasAnhSang && !hasSet) {
    ...
    public static void updateSetOptions(Char charz) {
        ...
        updateSetOption(isSet3AnhSang(charz), id_option_set_bo_3, charz);
        updateSetOption(isSet6AnhSang(charz), id_option_set_bo_6, charz);
    }
    ...
    public static void updateSetOptionsByItem(Char charz, Item trangBi) {
        ...
        updateSetOptionByItem(isSet3AnhSang(charz), id_option_set_bo_3, charz, trangBi);
        updateSetOptionByItem(isSet6AnhSang(charz), id_option_set_bo_6, charz, trangBi);
    }
    ...
    private static void calculateBaseParams(Char charz) {
        ...
        for (Item trangBi : charz.arrItemBody) {
            ...
            for (ItemOption op : trangBi.options) {
                System.out.println("trang bị ánh sáng " + op.optionTemplate.id + "param" + op.param);
            }
        }
    }
